...
function buildArgs(config) {
  let mongoConfParams = [`password: ${config.MONGO_PASSWORD}`];

  utils.writeMongoConfigFile(mongoConfParams);

  let args = [
    `--uri ${config.MONGO_URI}`,
    `--username ${config.MONGO_USER}`,
    `--config /tmp/mongo.conf`,
    "--ssl",
    "--drop",
    "--archive=/tmp/filename.backup",
  ];
  ...
  return new Promise((resolve, reject) => {
    ...
    process.on("close", (code) => {
      ...
      if (code !== 0) {
        return reject(
          new Error(
            `mognorestore process failed code: ${code} error: ${stderr}`
          )
        );
      }
